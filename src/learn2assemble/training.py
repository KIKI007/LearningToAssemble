...
from learn2assemble.agent import PPO, PPO_GAT
...
def policy_check(ppo_agent, curriculum_inds):
    ...
    wandb.log({"training accuracy": accuracy, "min_success": torch.min(ppo_agent.num_success)},
              step=ppo_agent.time_step)
    return accuracy
...
def train(ppo_agent, env, settings):

    training_setting = set_default(settings,
                "training",{
                    "max_train_epochs": 50000,
                })

    training_setting = SimpleNamespace(**training_setting)
    ...
    # training
    while ppo_agent.episode <= training_setting.max_train_epochs and ppo_agent.saved_accuracy < 1:

        curriculum_inds = sample_curriculum(ppo_agent, env.num_rollouts)
        ppo_agent.buffer.curriculum_inds = curriculum_inds.cpu()
        ppo_agent.buffer.rewards = rollout_asyn(ppo_agent, env, curriculum_inds).cpu()
